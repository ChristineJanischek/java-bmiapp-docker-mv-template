...
RUBRIC_DOCX="${2:-$ROOT_DIR/downloads/boegen/BK_Bewertung_Projekte_OOP_2026.docx}"
PROFILE_JSON="${3:-$ROOT_DIR/scripts/grading_profiles/bk_oop_2026_rubrik24.json}"
OUT_DIR="${4:-$ROOT_DIR/downloads/batch_bewertungen}"
ARCHIVE_DIR="${5:-$ROOT_DIR/downloads/archiv}"
TEACHER_NOTE="${6:-Automatische Erstbewertung, final manuell pruefen.}"

mkdir -p "$ZIP_DIR" "$OUT_DIR" "$ARCHIVE_DIR"

if [[ ! -f "$RUBRIC_DOCX" ]]; then
  echo "Fehler: Bewertungsbogen nicht gefunden: $RUBRIC_DOCX"
  echo "Lege den Bogen dort ab oder uebergib einen Pfad als 2. Parameter."
  exit 1
fi
