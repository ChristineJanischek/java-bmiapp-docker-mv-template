...
public void speichereMesswerteDatenbank(List<Temperaturmessung> messungen, String ordner) throws IOException {
    ...
    // Atomare Schreibweise mit Backup:
    Path schemaBak = Paths.get(schemaPfad.toString() + ".bak");
    Path datenBak = Paths.get(datenPfad.toString() + ".bak");

    try {
        // Backup erstellen, falls Dateien existieren
        if (Files.exists(schemaPfad)) {
            Files.copy(schemaPfad, schemaBak, StandardCopyOption.REPLACE_EXISTING);
        }
        if (Files.exists(datenPfad)) {
            Files.copy(datenPfad, datenBak, StandardCopyOption.REPLACE_EXISTING);
        }

        // Neue Dateien schreiben
        Files.writeString(schemaPfad, schemaJson, StandardCharsets.UTF_8);
        Files.writeString(datenPfad, datenJson.toString(), StandardCharsets.UTF_8);
    } catch (IOException e) {
        // Rollback bei Fehler
        if (Files.exists(schemaBak)) {
            Files.copy(schemaBak, schemaPfad, StandardCopyOption.REPLACE_EXISTING);
        }
        if (Files.exists(datenBak)) {
            Files.copy(datenBak, datenPfad, StandardCopyOption.REPLACE_EXISTING);
        }
        throw e;
    }
}
...
public List<Temperaturmessung> ladeMesswerteDatenbank(String ordner) throws IOException, IllegalArgumentException {
    ...
    double minValid = extractDouble(schema, "\"minValid\": ");
    double maxValid = extractDouble(schema, "\"maxValid\": ");
    ...
    List<Temperaturmessung> result = new ArrayList<>();

    // Extrahiere und validiere Messwerte (einfacher Parser)
    String[] entries = daten.split("\\{\"id\"");
    for (int i = 1; i < entries.length; i++) {
        // Extrahiere Felder...
        double temp = extractDouble(entries[i], "\"temperatur\": ");

        // Validierung der Temperatur
        if (temp < minValid || temp > maxValid) {
            throw new IllegalArgumentException(
                "Messwert " + temp + " liegt außerhalb des gültigen Bereichs [" + minValid + ", " + maxValid + "]"
            );
        }

        // Füge zu Ergebnis hinzu (Parsing-Details vereinfacht)
        result.add(new Temperaturmessung(/* ... */));
    }
...
```json
{
  ...
  "messungen": [
    {"id": 1, "temp": 22.5, "ort": "Kueche"},
    {"id": 2, "temp": 20.1, "ort": "Schlafzimmer"},
    {"id": 3, "temp": 18.9, "ort": "Buero"}
  ]
}
```
...
```java
for (Temperaturmessung m : messungen) {
    String zeitstempel = m.getZeitstempel();  // Aus dem Objekt verwenden
    json += "{\"zeitstempel\": \"" + zeitstempel + "\", ...}";
}
```

Oder bei neuem Speichern:
```java
String zeitstempel = LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME);
for (Temperaturmessung m : messungen) {
    json += "{\"zeitstempel\": \"" + zeitstempel + "\", ...}";
}
```
